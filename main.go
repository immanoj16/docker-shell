...
import (
	...
	"github.com/immanoj16/docker-shell/lib/commands"
)

var dockerClient *docker.Client

var shellCommands = commands.New()

// DockerHubResult used to wrap DockerHub API call
...
func imagesSuggestion() []prompt.Suggest {
	...
	for _, image := range images {
		...
		suggestions = append(suggestions, prompt.Suggest{
			Text:        image.ID[7:19],
			Description: getDescription(ins),
		})
	}
...
func portMappingSuggestion() []prompt.Suggest {
	...
	for _, image := range images {
		...
		for _, exposedPort := range exposedPortKeys {
			...
			suggestions = append(suggestions, prompt.Suggest{
				Text:        fmt.Sprintf("-p %s:%s:%s", port, port, portType),
				Description: getDescription(inspection),
			})
		}
	}
...
func main() {
	...
	for {
		...
		if splittedDockerCommands[0] == "clear" {
			...
			fmt.Println("here")
			fmt.Println(splittedDockerCommands)
